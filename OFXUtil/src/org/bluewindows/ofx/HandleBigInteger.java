...
public class HandleBigInteger extends AbstractTagHandler {

	//Set this class up as a Singleton
	public final static HandleBigInteger INSTANCE = new HandleBigInteger();
	protected HandleBigInteger() {}
	...
	public HandlerResult handle(OFXParser parser, AbstractAggregate aggregate, Tag tag) throws ParseException {
		...
		BigInteger contentObject = convertContentToObject(contentString, tag, parser);
		putContentObjectIntoTargetObject(aggregate, tag, contentObject);
		return new HandlerResult();
	}

	public BigInteger convertContentToObject(String amountString, Tag tag, OFXParser parser) throws ParseException {
		BigInteger contentObject;
		try {
			contentObject = new BigInteger(amountString);
		} catch (Exception e) {
			throw new ParseException("Invalid BigInteger value: [" + amountString + "] found in " +
