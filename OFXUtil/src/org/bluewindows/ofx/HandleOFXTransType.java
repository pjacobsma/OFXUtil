...
public class HandleOFXTransType extends AbstractTagHandler {

	//Set this class up as a Singleton
	public final static HandleOFXTransType INSTANCE = new HandleOFXTransType();
	protected HandleOFXTransType() {}
	...
	public HandlerResult handle(OFXParser parser, AbstractAggregate aggregate, Tag tag ) throws ParseException {
		...
		OFXTransType contentObject = convertContentToObject(contentString, tag, parser);
		putContentObjectIntoTargetObject(aggregate, tag, contentObject);
		return new HandlerResult();
	}

	@Override
	public OFXTransType convertContentToObject(String contentString, Tag tag, OFXParser parser) throws ParseException {
		OFXTransType contentObject;
		try {
			contentObject = OFXTransType.valueOf(contentString);
		} catch (IllegalArgumentException e) {
			throw new ParseException("Invalid Transaction Type: [" + contentString + "] found in " +
